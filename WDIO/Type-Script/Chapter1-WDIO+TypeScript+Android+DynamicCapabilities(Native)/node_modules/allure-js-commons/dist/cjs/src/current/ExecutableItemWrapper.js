...
class ExecutableItemWrapper {
    ...
    parameter(name, value, options) {
        this.info.parameters.push(Object.assign(Object.assign({}, options), { name, value: (0, utils_js_1.serialize)(value) }));
    }
    get isAnyStepFailed() {
        return (0, utils_js_1.isAnyStepFailed)(this.info);
    }
    get isAllStepsEnded() {
        return (0, utils_js_1.isAllStepsEnded)(this.info);
    }
    ...
    startStep(name, start) {
        const result = (0, constructors_js_1.stepResult)();
    ...
    wrap(fun) {
        return (...args) => {
            this.stage = model_js_1.Stage.RUNNING;
            let result;
            try {
                result = fun(args);
            }
            catch (error) {
                this.stage = model_js_1.Stage.INTERRUPTED;
                this.status = model_js_1.Status.BROKEN;
                if (error) {
            ...
            if ((0, isPromise_js_1.isPromise)(result)) {
                ...
                return promise
                    .then((res) => {
                    this.status = model_js_1.Status.PASSED;
                    this.stage = model_js_1.Stage.FINISHED;
                    return res;
                })
                    .catch((error) => {
                    this.stage = model_js_1.Stage.INTERRUPTED;
