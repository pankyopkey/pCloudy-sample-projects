...
class AppiumConnector(Connector):
    ...
    def initAppiumHubForBrowser(self, authToken, browserName):
        data = {"token": authToken, "browserName": browserName}
        print "Start Init " + time.strftime("%X")
        response = requests.post('https://device.pcloudy.com/api/appium/execute', headers=self.headers,
                                 data=json.dumps(data), verify=False)
        time.sleep(5)
...
try:
    auth_token = con.authenticateUser("Enter your Email-id", "Enter your API key")
...
bookedDevices = con.bookDeviceForAppium(auth_token, 10, "android", selected_device, session_name)

print "Devices booked succesfully"

con.initAppiumHubForBrowser(auth_token, 'Chrome')
